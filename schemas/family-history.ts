...
export const RelationshipEnum = z.enum(['parent', 'sibling', 'grandparent', 'child', 'aunt', 'uncle', 'cousin'])
...
export const FamilyHistoryRowSchema = z.object({
  ...
  age_at_onset: z.number().int().nullable(),
...
export const familyHistoryFormSchema = z.object({
  relative: z.string().optional(),
  condition: z.string().min(1, 'Medical condition is required').max(500),
  relationship: RelationshipEnum,
  age_at_onset: z.number().int().min(0).max(150).optional(),
  notes: z.string().optional(),
})
