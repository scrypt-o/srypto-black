...
const accentHex: Record<Exclude<VerticalTileProps['accent'], 'auto'>, string> = {
  indigo: '#6366f1',
  emerald: '#10b981',
  rose: '#f43f5e',
  amber: '#f59e0b',
...
export default function VerticalTile({ id, title, description, icon, href, badge, disabled, accent = 'auto' }: VerticalTileProps) {
  const ac = accentClasses[accent]
  const accentValue = accent === 'auto' ? '#94a3b8' : accentHex[accent]
  const tileStyle: CSSProperties = {
    // CSS variable for accent used in color-mix backgrounds
    ...
    borderColor: 'color-mix(in lab, var(--accent) 22%, #e9eef5)',
    boxShadow: '0 1px 0 rgba(255,255,255,.9), 0 10px 30px rgba(15, 35, 80, .12)'
  }
  const iconWrapStyle: CSSProperties = {
    background: 'conic-gradient(from 210deg, color-mix(in lab, var(--accent) 18%, #fff), white)',
    borderColor: 'color-mix(in lab, var(--accent) 30%, #dfe8ff)',
  ...
  const content = (
    <div
      className={clsx(
        ...
        'border bg-white dark:bg-gray-900 hover:shadow-[0_16px_38px_-14px_rgba(0,0,0,0.20)]',
    ...
    >
      ...
      />
      <div className="grid grid-cols-[52px,1fr,auto] gap-3 items-center">
        <div
          className={clsx('h-[52px] w-[52px] rounded-xl grid place-items-center', 'bg-white dark:bg-gray-950 border', ac.border)}
          style={iconWrapStyle}
        >
        ...
        <div className="min-w-0">
          ...
          {description && <p className="mt-1 text-[12px] sm:text-[13px] text-gray-600 dark:text-gray-400 truncate">{description}</p>}
        </div>
